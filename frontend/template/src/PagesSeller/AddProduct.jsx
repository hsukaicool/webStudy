...
export default function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 💡 呼叫更新後的 API
      await productApi.addProduct(payload, selectedFile);
      alert('🎉 商品已成功上架！');
      navigate('/seller/products');
    } catch (error) {
      console.error('上架失敗:', error);
      alert('上架失敗，請檢查權限或資料格式');
    } finally {
    ...
    <div className="add-product">
      ...
      <header className="add-product__header">
        ...
        <div className="add-product__header-text">
          <h1 className="add-product__title">新增商品</h1>
          <p className="add-product__subtitle">填寫商品詳細資訊以上架至您的賣場</p>
        </div>
      </header>
